...
default_cfgs = {
    ...
    "vit_custom_patch32_384": _cfg(
        url="",  # attempt at custom vit model
        mean=(0.5, 0.5, 0.5),
        std=(0.5, 0.5, 0.5),
    ),
...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False):
        ...
        select = torch.cat(select, dim=0).to('cuda')
        x = x[select[:, 0], select[:, 1]].view(B, -1, C)
        x_mask = x_mask[select[:, 0], select[:, 1]].view(B, -1)
...
@register_model
def vit_custom_patch32_384(pretrained=False, **kwargs):
    """ My custom 'small' ViT model. Depth=12, heads=4= mlp_ratio=3."""
    model_kwargs = dict(
        patch_size=32,
        embed_dim=256,
        depth=12,
    ...
    model = _create_vision_transformer(
        "vit_custom_patch32_384", pretrained=pretrained, **model_kwargs
    )
    return model
