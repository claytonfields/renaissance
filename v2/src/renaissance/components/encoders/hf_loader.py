...
Ported from the 1.3 line (`renaissance/modeling/backbones/hf_loader.py`).
Two functions:
...
def load_hf_encoder(
    ...
    *,
    ...
    if random_init:
        ...
        if overrides:
            raise ValueError(
                "overrides only apply when random_init=True; pretrained weights fix the architecture dims."
            )
