...
The ground-up 2.0 redesign (`docs/v2-design.md` is the source of truth) is
developed in-tree at `v2/` — a src-layout package with its own `pyproject.toml`
(PEP 621, Python >=3.10). Both lines use the import name `renaissance`, so
**never install v2 and 1.3 into the same environment**: use a dedicated venv
(`pip install -e "v2[dev]"`), and run v2 tests from inside `v2/` so its own
pytest config applies (`cd v2 && pytest`). The golden shape fixture under
`v2/tests/golden/` is generated by `scripts/gen_v2_golden_fixture.py` (run in a
1.3 env) — regenerate it, never hand-edit it. CI runs the v2 suite as the
`test-v2` job.
